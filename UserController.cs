...
namespace Clinic_WebAPI.Controllers
{
    ...
    public class UserController : ApiController
    {
        ...
        [HttpPost]
        [Route("api/User/EditUserStatus")]
        [JwtAuthentication]
        public APIResponse EditUserStatus(string userid, bool status)
        {
            //UserModel obj = new UserModel();
            Int64 UserID = -1;
            ...
            try
            {
                UserID = um.EditUserStatusByUserId(userid, status);
                if (UserID == 0)
                {
                    response.IsError = false;
        ...
        [HttpPost]
        [Route("api/User/EditPlatForm")]
        [JwtAuthentication]
        public APIResponse EditPlatForm(string userid, int platformtypeid)
        {
            //UserModel obj = new UserModel();
            Int64 UserID = -1;
            ...
            try
            {
                UserID = um.EditPlatFormByUserId(userid, platformtypeid);
                if (UserID ==0)
                {
                    response.IsError = false;
                    response.Data = UserID;
