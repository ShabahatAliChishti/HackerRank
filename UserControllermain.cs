...
namespace Clinic.Controllers.User
{
    public class UserController : BaseController
    {
        ...
        {
            if (Session["User"] != null || Session["SuperAdmin"]!=null)
            {
                try
                {
                    return View();
        ...
        public ActionResult EditUserStatus(string UserID,bool status)
        {
              ...
              Int64 recordId = -1;
                ...
                try
                {
                    if (UserID != null)
                    {
                        ...
                        var client = new RestClient(String.Concat(_apiUrl, "api/User/EditUserStatus?userid=" + UserID + "&status=" + status));
        ...
        public ActionResult EditPlatForm(string UserID, int platformtypeid)
        {
            ...
            try
            {
                if (UserID != null)
                {
                    ...
                    UserModel model = new UserModel();
                    ClinicBO.APIResponse apiResponse = new ClinicBO.APIResponse();
                    var client = new RestClient(String.Concat(_apiUrl, "api/User/EditPlatForm?userid=" + UserID + "&platformtypeid=" + platformtypeid));
                    var request = new RestRequest("", Method.POST) { RequestFormat = DataFormat.Json };
                    request.AddParameter("Authorization", string.Format("Bearer " + Session["token"].ToString()), ParameterType.HttpHeader);
                    var response = client.Execute<bool>(request);
                    if (response.StatusCode == System.Net.HttpStatusCode.OK && response.Content.Length > 0)
        ...
        public string SaveUserDetails(UserModel model)
        {
            ...
            try
            {
                ...
                {
                    usermodel = (LoginModel)Session["SuperAdmin"];
                    goto done;
                }
                done: model.Password = UtilityController.Encrypt(model.Password, "CCMClinic");
